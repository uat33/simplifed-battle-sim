 ...
 * Admittedly this part was not thought out in detail by myself before i started. 
...
public class Battle {
	...
	public Battle() throws InterruptedException {
		...
		do {
			...
			if(decision1 instanceof Individual && decision2 instanceof Individual) { // if both switch
				...
				((Move) decision2).purpose(pokemon2, pokemon1, (Move) decision2); // decision 2 must be a move, so cast it and call its purpose method
				...
				((Move) decision1).purpose(pokemon1, pokemon2, (Move) decision1); // decision 1 must be a move, so cast it and call its purpose method
				// so call the purpose method
			}
	...
	public static Individual newPokemon (Individual target, ArrayList<Individual> team) {  // takes the pokemon that died and its team as parameters
		...
		Setup.removePokemon(target, team); // use the method in setup 
	...
	public void attacks(Object firstMove, Object secondMove, Individual firstPokemon, Individual secondPokemon) throws InterruptedException {
		((Move) firstMove).purpose(firstPokemon, secondPokemon, (Move) firstMove);

		if (secondPokemon.isAlive()){ // if the secondPokemon is dead, it cannot attack
			((Move) secondMove).purpose(secondPokemon, firstPokemon, (Move) secondMove); // decision 2 must be a move
		}
	}
	...
	public Object playerTurn(Individual pokemon, int turns, ArrayList<Individual> team) {
		...
		// ok whatever, lets just put it in reverse order from what i want
